...
一个**纯前端 SPA**，归档「海垦日报」每日自动生成的 HTML 报告（四大板块：① 海南橡胶 & 海垦集团（海南自贸区 / 文昌航天城 / 经营人事）② 全国农垦遥感（各垦区数字化 + 长光 / 四维）③ 遥感 × 农业全球（含 Planet / Maxar / Airbus）④ 遥感技术（术语 / 前沿 / 历史案例）；深色海南配色、移动端适配）。零后端、零数据库、零构建工具——双击或起个静态服务就能跑。
...
├── styles.css              # 归档站自身样式（深色科技风 + 移动端适配）
├── data/
